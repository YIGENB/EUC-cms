        ...
        else
        {
            ...
            else
            {
                ...
                try
                {
                    ...
                    CShop.Entity.BaseConfigInfo config = CShop.Config.BaseConfig.Get();

                    if (config.WatermarkGoods && CShop.Common.Utils.FileExists(Server.MapPath(config.WatermarkPic)))
                    {
                        CShop.Common.WebGDI.GetWaterMarkPicImage(src, Server.MapPath(config.WatermarkPic), config.WatermarkPlace, Server.MapPath(target), 0, 0);
                    } 
                }
                catch (Exception ex)
                {
