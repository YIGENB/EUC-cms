...
function SelectZone()
{
  ...
  this.loadOptions = function(act, filters) {
      Ajax.call(this.filename + "&act=" + act + "&filter=" + filters, "", this.loadOptionsResponse, "GET", "JSON");
  }
...
this.addItem = function (all, act) {
    ...
    if (selOpt.length > 0)
    {
      ...
      Ajax.call(this.filename + "&act="+act+"&add_ids=" +selOpt+"&test="+args, args, this.addRemoveItemResponse, "GET", "JSON");
    }
  }
  ...
  this.dropItem = function(all, act) {
      ...
      if (arr.length > 0) {
          ...
          Ajax.call(this.filename + "&act=" + act + "&drop_ids=" + arr + "&test=" + args, args, this.addRemoveItemResponse, 'GET', 'JSON');
      }
  }
  ...
  this.createOptions = function(obj, arr) {
      ...
      obj.length = 0;
      for (var i = 0; i < arr[0].content.length; i++) {
          var opt = document.createElement("OPTION");
          opt.value = arr[0].content[i].value;
          opt.text = arr[0].content[i].text;
          opt.id = arr[0].content[i].data;
