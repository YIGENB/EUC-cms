...
public class imageUp : IHttpHandler
{
    public void ProcessRequest(HttpContext context)
    {
        if (!String.IsNullOrEmpty(context.Request.QueryString["fetch"]))
        {
            ...
            context.Response.Write(String.Format("updateSavePath([{0}]);", String.Join(",", UeditorConfig.ImageSavePath.Select(x => "\"" + x + "\"").ToArray())));
            return;
        }
        ...
        {
            path = UeditorConfig.ImageSavePath[0];
        }
        else if (UeditorConfig.ImageSavePath.Count(x => x == path) == 0)
        {
