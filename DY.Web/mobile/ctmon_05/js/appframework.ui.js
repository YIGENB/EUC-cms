 ...
 */ (function ($) {
         ...
         */
         ...
         addDivAndScroll: function (tmp, refreshPull, refreshFunc, container) {
             ...
             if (hasScroll) {
                 ...
                 //无限加载
                 var infinite = $("#" + scrollEl.id + " .infiniteScroll");
                 if (infinite.length > 0) {
                     var myScroller = $.ui.scrollingDivs[scrollEl.id];
                     myScroller.addInfinite();
                     $.bind(myScroller, "infinite-scroll", function () {
                         var self = this;
                         //ax
                         var arr = eval($("#" + $.ui.activeDiv.id + " .pvernextarr>*").text());
                         if (arr.next.url) { $.ui.showMask("加载中....");};                      
                             ...
                             if (arr.next.url) {
                                 $.ajax({
                                     ...
                                     success: function (data) {
                                         infinite.append($(data).find(".infiniteScroll>*"));
                                         $("#" + $.ui.activeDiv.id + " .pvernextarr").html("").append($(data).find(".pvernextarr>*"));
                                         $.ui.hideMask();
                                     }
                                 })
                             }
         ...
         launch: function () {
             ...
             //$(this.header).html('<a id="backButton" class="icon back"></a>' + this.header.innerHTML);
     ...
     var checkAnchorClick = function (e, theTarget) {
         ...
         if (theTarget.tagName !== "undefined" && theTarget.tagName.toLowerCase() == "a") {
             var theTargetAjax = theTarget.getAttribute("data-load-ajax");//ax1
             if (!theTargetAjax) return false;//ax2
