...
namespace DY.Data.SqlServer
{
    public partial class DataProvider : IDataProvider
    {
        ...
        public int InsertCshTypeInfo(CshTypeInfo entity)
        {
            Type entityType = entity.GetType();
            PropertyInfo[] propertyInfos = entityType.GetProperties();
            StringBuilder sb = new StringBuilder("INSERT INTO {0}csh_type ("), sbParamer = new StringBuilder(" (");
        ...
        public void UpdateCshTypeInfo(CshTypeInfo entity)
        {
            Type entityType = entity.GetType();
            PropertyInfo[] propertyInfos = entityType.GetProperties();
            StringBuilder sb = new StringBuilder("UPDATE {0}csh_type SET ");
            List<DbParameter> paramerList = new List<DbParameter>(); //参数集合
            foreach (PropertyInfo property in propertyInfos)
            {
                object oval = property.GetValue(entity, null);
        ...
        /// <returns></returns>
        public DbDataReader GetCshTypeInfo(int type_id)
        {
            string SQL = string.Format("SELECT * FROM {0}csh_type WHERE [type_id]=@type_id", BaseConfig.TablePrefix);
        ...
        /// <returns></returns>
        public DbDataReader GetCshTypeInfo(string filter)
        {
            if (!string.IsNullOrEmpty(filter))
                filter = "and " + filter;

            string SQL = string.Format("SELECT * FROM {0}csh_type WHERE [type_id]>0 {1}", BaseConfig.TablePrefix, filter);
        ...
        /// <param name="ad_id"></param>
        /// <returns></returns>
        public void DeleteCshTypeInfo(int type_id)
        {
            string SQL = string.Format("DELETE FROM {0}csh_type WHERE [type_id]=@type_id", BaseConfig.TablePrefix);
        ...
        /// <param name="filter"></param>
        /// <returns></returns>
        public bool ExistsCshType(string filter)
        {
            if (!string.IsNullOrEmpty(filter))
                filter = "and " + filter;

            string SQL = string.Format("SELECT COUNT(type_id) FROM {0}csh_type WHERE [type_id]>0 {1}", BaseConfig.TablePrefix, filter);
        ...
        /// <param name="filter"></param>
        /// <returns></returns>
        public object GetCshTypeValue(string Fields, string filter)
        {
            if (!string.IsNullOrEmpty(filter))
                filter = "and " + filter;

            string SQL = string.Format("SELECT {2} FROM {0}csh_type WHERE [type_id]>0 {1}", BaseConfig.TablePrefix, filter,Fields);
