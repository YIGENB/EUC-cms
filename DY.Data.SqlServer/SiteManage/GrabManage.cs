...
namespace DY.Data.SqlServer
{
    public partial class DataProvider : IDataProvider
    {
        ...
        public int InsertGrabInfo(Grab2Info entity)
        {
            Type entityType = entity.GetType();
            PropertyInfo[] propertyInfos = entityType.GetProperties();
            StringBuilder sb = new StringBuilder("INSERT INTO {0}grab_2 ("), sbParamer = new StringBuilder(" (");
        ...
        public void UpdateGrab2Info(Grab2Info entity)
        {
            Type entityType = entity.GetType();
            PropertyInfo[] propertyInfos = entityType.GetProperties();
            StringBuilder sb = new StringBuilder("UPDATE {0}grab_2 SET ");
            List<DbParameter> paramerList = new List<DbParameter>(); //参数集合
            foreach (PropertyInfo property in propertyInfos)
            {
                object oval = property.GetValue(entity, null);
        ...
        /// <returns></returns>
        public DbDataReader GetGrab2Info(int id)
        {
            string SQL = string.Format("SELECT * FROM {0}grab_2 WHERE [id]=@id", BaseConfig.TablePrefix);
        ...
        /// <returns></returns>
        public DbDataReader GetGrab2Info(string filter)
        {
            if (!string.IsNullOrEmpty(filter))
                filter = "and " + filter;

            string SQL = string.Format("SELECT * FROM {0}grab_2 WHERE [id]>0 {1}", BaseConfig.TablePrefix, filter);
        ...
        /// <param name="ad_id"></param>
        /// <returns></returns>
        public void DeleteGrab2Info(int id)
        {
            string SQL = string.Format("DELETE FROM {0}grab_2 WHERE [id]=@id", BaseConfig.TablePrefix);
        ...
        /// <param name="filter"></param>
        /// <returns></returns>
        public bool ExistsGrab2(string filter)
        {
            if (!string.IsNullOrEmpty(filter))
                filter = "and " + filter;

            string SQL = string.Format("SELECT COUNT(id) FROM {0}grab_2 WHERE [id]>0 {1}", BaseConfig.TablePrefix, filter);
        ...
        /// <param name="filter"></param>
        /// <returns></returns>
        public object GetGrab2Value(string Fields, string filter)
        {
            if (!string.IsNullOrEmpty(filter))
                filter = "and " + filter;

            string SQL = string.Format("SELECT {2} FROM {0}grab_2 WHERE [id]>0 {1}", BaseConfig.TablePrefix, filter, Fields);
