...
namespace DY.Data.SqlServer
{
    public partial class DataProvider : IDataProvider
    {
        ...
        public int InsertAdminUserInfo(AdminUserInfo entity)
        {
            Type entityType = entity.GetType();
            PropertyInfo[] propertyInfos = entityType.GetProperties();
            StringBuilder sb = new StringBuilder("INSERT INTO {0}admin_user ("), sbParamer = new StringBuilder(" (");
        ...
        public void UpdateAdminUserInfo(AdminUserInfo entity)
        {
            Type entityType = entity.GetType();
            PropertyInfo[] propertyInfos = entityType.GetProperties();
            StringBuilder sb = new StringBuilder("UPDATE {0}admin_user SET ");
            List<DbParameter> paramerList = new List<DbParameter>(); //参数集合
            foreach (PropertyInfo property in propertyInfos)
            {
                object oval = property.GetValue(entity, null);
        ...
        /// <returns></returns>
        public DbDataReader GetAdminUserInfo(int user_id)
        {
            string SQL = string.Format("SELECT * FROM {0}admin_user WHERE [user_id]=@user_id", BaseConfig.TablePrefix);
        ...
        /// <returns></returns>
        public DbDataReader GetAdminUserInfo(string filter)
        {
            if (!string.IsNullOrEmpty(filter))
                filter = "and " + filter;

            string SQL = string.Format("SELECT * FROM {0}admin_user WHERE [user_id]>0 {1}", BaseConfig.TablePrefix, filter);
        ...
        /// <param name="ad_id"></param>
        /// <returns></returns>
        public void DeleteAdminUserInfo(int user_id)
        {
            string SQL = string.Format("DELETE FROM {0}admin_user WHERE [user_id]=@user_id", BaseConfig.TablePrefix);
        ...
        /// <param name="filter"></param>
        /// <returns></returns>
        public bool ExistsAdminUser(string filter)
        {
            if (!string.IsNullOrEmpty(filter))
                filter = "and " + filter;

            string SQL = string.Format("SELECT COUNT(user_id) FROM {0}admin_user WHERE [user_id]>0 {1}", BaseConfig.TablePrefix, filter);
        ...
        /// <param name="filter"></param>
        /// <returns></returns>
        public object GetAdminUserValue(string Fields, string filter)
        {
            if (!string.IsNullOrEmpty(filter))
                filter = "and " + filter;

            string SQL = string.Format("SELECT {2} FROM {0}admin_user WHERE [user_id]>0 {1}", BaseConfig.TablePrefix, filter,Fields);
