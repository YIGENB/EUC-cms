...
namespace DY.Data.SqlServer
{
    public partial class DataProvider : IDataProvider
    {
        ...
        public int InsertCmsLinkInfo(CmsLinkInfo entity)
        {
            Type entityType = entity.GetType();
            PropertyInfo[] propertyInfos = entityType.GetProperties();
            StringBuilder sb = new StringBuilder("INSERT INTO {0}cms_link ("), sbParamer = new StringBuilder(" (");
        ...
        public void UpdateCmsLinkInfo(CmsLinkInfo entity)
        {
            Type entityType = entity.GetType();
            PropertyInfo[] propertyInfos = entityType.GetProperties();
            StringBuilder sb = new StringBuilder("UPDATE {0}cms_link SET ");
            List<DbParameter> paramerList = new List<DbParameter>(); //参数集合
            foreach (PropertyInfo property in propertyInfos)
            {
                object oval = property.GetValue(entity, null);
        ...
        /// <returns></returns>
        public DbDataReader GetCmsLinkInfo(int link_id)
        {
            string SQL = string.Format("SELECT * FROM {0}cms_link WHERE [link_id]=@link_id", BaseConfig.TablePrefix);
        ...
        /// <returns></returns>
        public DbDataReader GetCmsLinkInfo(string filter)
        {
            if (!string.IsNullOrEmpty(filter))
                filter = "and " + filter;

            string SQL = string.Format("SELECT * FROM {0}cms_link WHERE [link_id]>0 {1}", BaseConfig.TablePrefix, filter);
        ...
        /// <param name="ad_id"></param>
        /// <returns></returns>
        public void DeleteCmsLinkInfo(int link_id)
        {
            string SQL = string.Format("DELETE FROM {0}cms_link WHERE [link_id]=@link_id", BaseConfig.TablePrefix);
        ...
        /// <param name="filter"></param>
        /// <returns></returns>
        public bool ExistsCmsLink(string filter)
        {
            if (!string.IsNullOrEmpty(filter))
                filter = "and " + filter;

            string SQL = string.Format("SELECT COUNT(link_id) FROM {0}cms_link WHERE [link_id]>0 {1}", BaseConfig.TablePrefix, filter);
        ...
        /// <param name="filter"></param>
        /// <returns></returns>
        public object GetCmsLinkValue(string Fields, string filter)
        {
            if (!string.IsNullOrEmpty(filter))
                filter = "and " + filter;

            string SQL = string.Format("SELECT {2} FROM {0}cms_link WHERE [link_id]>0 {1}", BaseConfig.TablePrefix, filter,Fields);
