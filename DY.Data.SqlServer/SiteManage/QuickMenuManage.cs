...
namespace DY.Data.SqlServer
{
    public partial class DataProvider : IDataProvider
    {
        ...
        public int InsertQuickMenuInfo(QuickMenuInfo entity)
        {
            Type entityType = entity.GetType();
            PropertyInfo[] propertyInfos = entityType.GetProperties();
            StringBuilder sb = new StringBuilder("INSERT INTO {0}quick_menu ("), sbParamer = new StringBuilder(" (");
        ...
        public void UpdateQuickMenuInfo(QuickMenuInfo entity)
        {
            Type entityType = entity.GetType();
            PropertyInfo[] propertyInfos = entityType.GetProperties();
            StringBuilder sb = new StringBuilder("UPDATE {0}quick_menu SET ");
            List<DbParameter> paramerList = new List<DbParameter>(); //参数集合
            foreach (PropertyInfo property in propertyInfos)
            {
                object oval = property.GetValue(entity, null);
        ...
        /// <returns></returns>
        public DbDataReader GetQuickMenuInfo(int id)
        {
            string SQL = string.Format("SELECT * FROM {0}quick_menu WHERE [id]=@id", BaseConfig.TablePrefix);
        ...
        /// <returns></returns>
        public DbDataReader GetQuickMenuInfo(string filter)
        {
            if (!string.IsNullOrEmpty(filter))
                filter = "and " + filter;

            string SQL = string.Format("SELECT * FROM {0}quick_menu WHERE [id]>0 {1}", BaseConfig.TablePrefix, filter);
        ...
        /// <param name="ad_id"></param>
        /// <returns></returns>
        public void DeleteQuickMenuInfo(int id)
        {
            string SQL = string.Format("DELETE FROM {0}quick_menu WHERE [id]=@id", BaseConfig.TablePrefix);
        ...
        /// <param name="filter"></param>
        /// <returns></returns>
        public bool ExistsQuickMenu(string filter)
        {
            if (!string.IsNullOrEmpty(filter))
                filter = "and " + filter;

            string SQL = string.Format("SELECT COUNT(id) FROM {0}quick_menu WHERE [id]>0 {1}", BaseConfig.TablePrefix, filter);
        ...
        /// <param name="filter"></param>
        /// <returns></returns>
        public object GetQuickMenuValue(string Fields, string filter)
        {
            if (!string.IsNullOrEmpty(filter))
                filter = "and " + filter;

            string SQL = string.Format("SELECT {2} FROM {0}quick_menu WHERE [id]>0 {1}", BaseConfig.TablePrefix, filter,Fields);
