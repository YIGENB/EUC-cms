...
namespace DY.Data.SqlServer
{
    public partial class DataProvider : IDataProvider
    {
        ...
        public int InsertOrderGoodsInfo(OrderGoodsInfo entity)
        {
            Type entityType = entity.GetType();
            PropertyInfo[] propertyInfos = entityType.GetProperties();
            StringBuilder sb = new StringBuilder("INSERT INTO {0}order_goods ("), sbParamer = new StringBuilder(" (");
        ...
        public void UpdateOrderGoodsInfo(OrderGoodsInfo entity)
        {
            Type entityType = entity.GetType();
            PropertyInfo[] propertyInfos = entityType.GetProperties();
            StringBuilder sb = new StringBuilder("UPDATE {0}order_goods SET ");
            List<DbParameter> paramerList = new List<DbParameter>(); //参数集合
            foreach (PropertyInfo property in propertyInfos)
            {
                object oval = property.GetValue(entity, null);
        ...
        /// <returns></returns>
        public DbDataReader GetOrderGoodsInfo(int rec_id)
        {
            string SQL = string.Format("SELECT * FROM {0}order_goods WHERE [rec_id]=@rec_id", BaseConfig.TablePrefix);
        ...
        /// <returns></returns>
        public DbDataReader GetOrderGoodsInfo(string filter)
        {
            if (!string.IsNullOrEmpty(filter))
                filter = "and " + filter;

            string SQL = string.Format("SELECT * FROM {0}order_goods WHERE [rec_id]>0 {1}", BaseConfig.TablePrefix, filter);

            return DbHelper.ExecuteReader(CommandType.Text, SQL);
        }
        /// <summary>
        /// 删除OrderGoods信息（按主键删除）
        /// </summary>
        /// <param name="ad_id"></param>
        /// <returns></returns>
        public void DeleteOrderGoodsInfo(int rec_id)
        {
            string SQL = string.Format("DELETE FROM {0}order_goods WHERE [rec_id]=@rec_id", BaseConfig.TablePrefix);
        ...
        /// <param name="filter"></param>
        /// <returns></returns>
        public bool ExistsOrderGoods(string filter)
        {
            if (!string.IsNullOrEmpty(filter))
                filter = "and " + filter;

            string SQL = string.Format("SELECT COUNT(rec_id) FROM {0}order_goods WHERE [rec_id]>0 {1}", BaseConfig.TablePrefix, filter);
        ...
        /// <param name="filter"></param>
        /// <returns></returns>
        public object GetOrderGoodsValue(string Fields, string filter)
        {
            if (!string.IsNullOrEmpty(filter))
                filter = "and " + filter;

            string SQL = string.Format("SELECT {2} FROM {0}order_goods WHERE [rec_id]>0 {1}", BaseConfig.TablePrefix, filter,Fields);
