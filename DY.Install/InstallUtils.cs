...
namespace DY.Install
{
    public class InstallUtils
    {
        ...
        public static string InitialSystemValidCheck()
        {
            ...
            string fileName = context != null ? context.Server.MapPath("/config/CTMON.config") : Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "config/CTMON.config");
            ...
            {
                sb.AppendFormat(messageTemplate, "false", "/config/CTMON.config 不可写或没有放置正确, 相关问题详见安装文档!");
            }
            else
                sb.AppendFormat(messageTemplate, "true", "/config/CTMON.config 验证通过!");

            //检查系统目录的有效性
            string folderstr = "backup,config,include,skin,include\\upload\\kind\\image,include\\upload\\kind\\file";
        ...
        public static bool GetRootDntconfigPath()
        {
            try
            {
                ...
                if (!Utils.FileExists(webconfigfile = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "config/CTMON.config"))
                    && (!Utils.FileExists(webconfigfile = Utils.GetMapPath("~/config/CTMON.config")))
                    && (!Utils.FileExists(webconfigfile = Path.Combine(context.Request.PhysicalApplicationPath, "config/CTMON.config")))
                    && (!Utils.FileExists(webconfigfile = Utils.GetMapPath("../config/CTMON.config")))
                    && (!Utils.FileExists(webconfigfile = Utils.GetMapPath("../../config/CTMON.config")))
                    && (!Utils.FileExists(webconfigfile = Utils.GetMapPath("../../../config/CTMON.config"))))
                {
        ...
        {
            StringBuilder sb = new StringBuilder();
            using (StreamReader objReader = new StreamReader(Utils.GetMapPath(dbScriptPath + "setup1.sql"), Encoding.UTF8))
            {
                sb.Append(objReader.ReadToEnd());
                objReader.Close();
            }

            if (BaseConfigs.GetTablePrefix.ToLower() == BaseConfig.TablePrefix)
                DbHelper.ExecuteCommandWithSplitter(sb.ToString());
            else
                DbHelper.ExecuteCommandWithSplitter(sb.ToString().Replace(BaseConfig.TablePrefix, BaseConfigs.GetTablePrefix));
        ...
        {
                StringBuilder sb = new StringBuilder();
                using (StreamReader objReader = new StreamReader(Utils.GetMapPath(dbScriptPath + "create.sql"), Encoding.UTF8))
                {
                    sb.Append(objReader.ReadToEnd());
                    objReader.Close();
                }
        ...
        public static string CreateStorePocedure()
        {
            ...
            if (BaseConfigs.GetTablePrefix.ToLower() == BaseConfig.TablePrefix)
                ...
                DbHelper.ExecuteCommandWithSplitter(sb.ToString().Trim().Replace("\"", "'").Replace(BaseConfig.TablePrefix, BaseConfigs.GetTablePrefix));
        ...
        public static string InitialForumSource(string adminName, string adminPassword)
        {
            try
            {
                ...
                //else
                //    DbHelper.ExecuteNonQuery(CommandType.Text, sb.ToString().Replace(BaseConfig.TablePrefix, BaseConfigs.GetTablePrefix));

                    DbHelper.ExecuteNonQuery(CommandType.Text, string.Format("truncate table [{0}admin_user]",
                        BaseConfigs.GetTablePrefix));
                    DbHelper.ExecuteNonQuery(CommandType.Text, string.Format("INSERT  [{0}admin_user] ([user_name], [email], [password], [add_time], [last_login], [last_ip], [action_list], [nav_list], [lang_type], [agency_id], [todolist]) VALUES ('{1}', '421643133@qq.com', '{2}', '1949-10-01', '1949-10-01', '127.0.0.1', 'all', '', '', 0, '')", BaseConfigs.GetTablePrefix, adminName, SiteUtils.Encryption(adminPassword)));
                ...
                //生成安装锁定文件
                string lockPath = Utils.GetMapPath("/install/lock.lock");
                string content = "欢迎使用创同盟营销系统\n安装时间：" + DateTime.Now;
                FileOperate.WriteFile(lockPath, content);
        ...
        {
            DeleteTable();
            BaseDataInfo baseConfig = BaseConfigs.GetBaseConfig();
            ...
            string dntPath = Utils.GetMapPath("~/config/CTMON.config");
            if (!Utils.FileExists(dntPath))
            {
                dntPath = Utils.GetMapPath("/config/CTMON.config");
            }
