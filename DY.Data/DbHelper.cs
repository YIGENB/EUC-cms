...
namespace DY.Data
{
                ...
                if (m_provider == null)
                {
                    lock(lockHelper)
                    {
                        if (m_provider == null)
                        {
                            try
                            {
                                m_provider = (IDbProvider)Activator.CreateInstance(Type.GetType(string.Format("DY.Data.{0}Provider, DY.Data.{0}", ConfigurationManager.AppSettings["dbType"]), false, true));
                            }
                            catch
                            {
	...
	#region ExecuteNonQuery方法
        ...
        public static int ExecuteNonQuery(out int id, DbConnection connection, CommandType commandType, string commandText, params DbParameter[] commandParameters)
        {
            ...
            DateTime dt1 = DateTime.Now;
#endif
            id = Utils.StrToInt((cmd.ExecuteScalar()),1);
#if DEBUG                
            DateTime dt2 = DateTime.Now;
        ...
        public static int ExecuteNonQuery(out int id, DbTransaction transaction, CommandType commandType, string commandText, params DbParameter[] commandParameters)
        {
            ...
            cmd.CommandType = CommandType.Text;
            cmd.CommandText = Provider.GetLastIdSql();
            id = Utils.StrToInt(cmd.ExecuteScalar().ToString(),0);
            return retval;
        }
	...
	#region 生成参数
        ...
        #region 生成数据类型
        public static DbType MakeDbType(object pValue)
        {
            return Provider.MakeDbType(pValue.GetType());
        }
        #endregion
