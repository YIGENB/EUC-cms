...
using DY.OAuthV2SDK.OAuths.Tencs.Models;

namespace DY.OAuthV2SDK.OAuths.Tencs
{
    /// <summary>
    /// 腾讯微博协议
    /// </summary>
    public class TencOAuth : OAuthBase
    {
        #region 属性
        ...
        /// 协议节点名称(区分大小写)
        /// </summary>
        public override string OAuthName { get { return "tenc"; } }
        /// <summary>
        /// 协议节点描述
        /// </summary>
        public override string OAuthDesc { get { return "腾讯微博"; } }
        ...
        public TencOAuth(string cfgAppName)
        {
            this.ClientIp = UtilHelper.GetClientIP();
        }
        ...
        public override ApiToken GetAccessToken()
        {
            ...
            else
            {
                ...
                {
                    this.AccessToken = qs[OAuthAccessTokenKey];
                ...
                {
                    this.RefreshToken = qs[OAuthRefreshTokenKey];
        ...
        public override ApiResult SendStatus(string accessToken, string strText)
        {
            ...
            string response = ApiByHttpPost("t_add", paras);
            TencMTweet status = UtilHelper.ParseJson<TencMTweet>(response);
            ApiResult api = new ApiResult();
            api.response = response;
            api.request = "t_add";
            if (status.ret == 0)
            ...
            {
                api.ret = 1;
                api.errcode = Convert.ToString(status.errcode);
                api.msg = status.msg;
            }
            return api;
        }
        ...
        public override ApiResult SendStatusWithPic(string accessToken, string strText, string strFile)
        {
            ...
            paras.Add("format", "json");
            paras.Add("content", strText);
            string response = ApiByHttpPostWithPic("t_add_pic", paras, files);
            TencMTweet status = UtilHelper.ParseJson<TencMTweet>(response);
            ApiResult api = new ApiResult();
            api.response = response;
            api.request = "t_add_pic";
            if (status.ret == 0)
            {
